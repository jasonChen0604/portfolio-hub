...
const t = {
	...
	zh: {
		...
		sec2: "資料格式",
		sec2body:
			"tech-profile/ 目錄包含一組扁平化 JSON 檔案，全部提供英文與繁體中文版本：",
		sec2items: [
		...
		sec3: "呈現方式",
		sec3body:
			"網站採用 Next.js 15 靜態輸出（output: 'export'）並部署至 Firebase Hosting。所有資料在建置時透過靜態 JSON import 載入，無需伺服器端 fetch 或資料庫。Joy UI 的 CssVarsProvider 以 CSS 變數處理深淺色切換，語言切換（EN ↔ 中文）則使用以 localStorage 為底的輕量 React Context。",

		sec4: "技術棧",

		sec5: "SEO 與 AI 友善",
		sec5body:
			"每個頁面皆匯出伺服器渲染的 metadata（title、description、OpenGraph）。產品詳細頁包含 JSON-LD 結構化資料（schema.org/SoftwareApplication），讓搜尋引擎與 AI 爬蟲能理解每個產品的上下文。/llms.txt 檔案遵循新興的 LLM 爬蟲慣例，支援 AI Agent 直接讀取。",
	},
};
