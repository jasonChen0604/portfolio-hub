...
import { domainVisualConfigs, categoryConfigs } from "@/lib/skills/hierarchyConfig";
...
const t = {
  en: { title: "Skills & Expertise", subtitle: "10 domains · click any skill to explore" },
...
export function SkillsClient({
  ...
  const [tooltip, setTooltip] = useState<{ data: SkillTooltipData; anchor: Element } | null>(null);
  const [activeDomain, setActiveDomain] = useState<string>(domainVisualConfigs[0].domainId);
  ...
  useEffect(() => {
    ...
    for (const cfg of domainVisualConfigs) {
      ...
      for (const cat of categoryConfigs.filter((c) => c.domainId === cfg.domainId)) {
        targets.push({ id: `cat-${cat.id}`, domainId: cfg.domainId, catId: cat.id });
      }
    }
    ...
    const observer = new IntersectionObserver(
      () => {
        ...
        if (best) {
          if (best.domainId !== activeRef.current.domain) setActiveDomain(best.domainId);
  ...
  const handleSkillClick = useCallback((data: SkillTooltipData, anchor: Element) => {
    ...
    <>
      ...
      >
        <motion.div
          ...
          style={{ flex: 1 }}
        >
          <List size="sm" sx={{ "--List-gap": "0px", "--ListItem-paddingY": "2px", py: 1 }}>
              ...
              return (
                <Box key={cfg.domainId}>
                  ...
                  <ListItem>
                    ...
                    >
                      <span style={{ marginRight: 4 }}>{rawDomain?.icon ?? cfg.icon}</span>
                      {domLabel}
                    </ListItemButton>
                  </ListItem>
                        ...
                        <ListItemButton
                          ...
                          sx={{
                            ...
                            "&::before": {
                              ...
                              borderRadius: "50%",
                              bgcolor: isCatActive ? cfg.color : "neutral.400",
                              mr: 1,
